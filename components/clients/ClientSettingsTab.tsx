...
/**
 * Client profile view/edit plus the delete action. Previously the whole
 * `/clients/[id]` page; now a tab inside `ClientsScreen` so there is a single
 * client surface. `/clients/{id}` deep-links here.
 */
export default function ClientSettingsTab({
  clientId, onUpdated, onDeleted,
}: {
  clientId: string;
  onUpdated?: (name: string, status: string) => void;
  onDeleted?: () => void;
}) {
  ...
  async function saveEdit() {
    ...
    try {
      ...
      onUpdated?.(res.data.data.name, res.data.data.status);
      cancelEdit();
      toast.success("Client updated");
    } catch (err) {
      const msg = (err as { response?: { data?: { message?: string } } })
        .response?.data?.message;
      toast.error(msg ?? "Failed to update client");
    } finally {
      setSaving(false);
    }
  }
